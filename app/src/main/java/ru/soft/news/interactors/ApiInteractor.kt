...
open class ApiInteractorImp(private val api: Service) : ApiInteractor {
    ...
    override fun getNewsList(): Single<List<News>> =
            api
                    ...
                    .map {
                        it.payload
                                .map {
                                    News(id = it.id, title = it.text, publicationDate = it.publicationDate.milliseconds)
                                }
                                .sortedByDescending {
                                    it.publicationDate
                                }
    ...
    override fun getNewsContent(newsId: String): Single<NewsContent> =
            api
                    ...
                    .map {
                        NewsContent(id = it.payload.title.id, title = it.payload.title.text, content = it.payload.content)
                    }
